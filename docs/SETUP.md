...
.venv/bin/pip install fastapi uvicorn requests pyyaml numpy anthropic \
    RealtimeSTT faster-whisper silero-vad websockets psutil pypdf python-docx python-multipart
...
**Job search (JOB_AGENT)**: install the second bundled plugin so "búscame
trabajo cerca de casa" actually filters/ranks real postings instead of
Hermes guessing distances and requirements itself — see
[docs/JOB_AGENT.md](JOB_AGENT.md) for the full picture:

```bash
cp -R hermes-plugin/job_search ~/.hermes/plugins/job_search
hermes plugins enable job_search
```

Then add `job_search` next to `hud` in `platform_toolsets` and
`known_plugin_toolsets` for every platform you use, in `~/.hermes/config.yaml`
(`hermes config edit`) — plugin toolsets aren't auto-allowlisted per platform,
this is not optional, the tools stay invisible to the agent without it. Restart
the gateway. Fill in `~/.hermes/memories/job_profile.yaml` (auto-created empty
on first call) with your real location/radius/skills/salary floor whenever you
want filtering to use them instead of falling back to general memory.
