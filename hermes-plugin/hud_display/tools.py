...
YT_PLAY_URL = os.environ.get("JARVIS_YT_PLAY_URL", "http://127.0.0.1:8765/api/yt_play")


def youtube_play(args: dict, **kwargs) -> str:
    query = (args.get("query") or "").strip()
    if not query:
        return json.dumps({"error": "query is required"})
    token = os.environ.get("JARVIS_HUD_TOKEN", "")
    req = urllib.request.Request(
        YT_PLAY_URL, data=json.dumps({"query": query}).encode(), method="POST",
        headers={"Content-Type": "application/json", "X-Jarvis-Token": token},
    )
    try:
